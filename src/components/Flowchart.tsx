...
const NODE_CONFIG = {
  ...
  condition: {
    ...
    textColor: "#2b5f8a",
    borderRadius: 25,
    shadowColor: "rgb(125 184 232 / 0.25)",
  },
  root: {
    width: 220,
    height: 100,
...
const FIT_VIEW_ZOOM = 0.5;
const FIT_VIEW_TOP_PADDING = 32;
...
export function Flowchart({
  ...
  const notedNodes = useMemo(() => {
    if (!selectedPath)
      return [] as { id: string; note: string; citation: number }[];
  ...
  const getNodeDimensions = (
    ...
    const subtitleHeight =
      type === "path-reference" ? PATH_REFERENCE_SUBTITLE_HEIGHT : 0;
    ...
    const height = Math.max(
      config.height,
      Math.ceil(
        lines.length * NODE_TEXT_LINE_HEIGHT +
  ...
  const measureSubtreeWidth = (
    ...
    if (currentNode.type === "condition" && currentNode.next) {
      return Math.max(
        ...
        measureSubtreeWidth(currentNode.next, resolved.visitedPaths),
      );
    }
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      setZoom((prev) =>
        Math.min(
          Math.max(
            prev * (e.deltaY > 0 ? ZOOM_OUT_FACTOR : ZOOM_IN_FACTOR),
            0.1,
          ),
          5,
        ),
  ...
  const getFootnoteRenderLines = (
    ...
    if (pathNote) {
      lines.push(
        ...wrapText(ctx, pathNote, maxTextWidth, Number.POSITIVE_INFINITY),
      );
    }
  ...
  const handleZoomToFit = () => {
    ...
    const targetZoom = FIT_VIEW_ZOOM;
    const contentCenterX = (minX + maxX) / 2;
    const newPanX = -contentCenterX * targetZoom;
    const newPanY = FIT_VIEW_TOP_PADDING - (minY + 50) * targetZoom;
  ...
  const handleExportPNG = () => {
    ...
    const footnotesHeight = getFootnotePanelHeight(
      exportCtx,
      contentWidth - 24,
    );
    const width = contentWidth;
    const height =
      contentHeight + (footnotesHeight > 0 ? footnotesHeight + 16 : 0);
  ...
  const handleExportSVG = () => {
    ...
    const notesBlockHeight =
      pathNote || notedNodes.length > 0
        ? 16 + notesLines.length * notesLineHeight + 12
        : 0;
    const width = contentWidth;
    const height =
      contentHeight + (notesBlockHeight > 0 ? notesBlockHeight + 16 : 0);
    const offsetX = -minX + padding;
    const offsetY = -minY + padding;
...
`;
      ...
      notesLines.forEach((line, index) => {
        const color =
          index === 0 ? "oklch(0.40 0.10 70)" : "oklch(0.30 0.03 260)";
        const family =
          index === 0 ? "Space Grotesk, sans-serif" : "Inter, sans-serif";
    ...
    >
      ...
      {allowFullscreen && (
        <Dialog open={isFullscreenOpen} onOpenChange={setIsFullscreenOpen}>
          <DialogContent className="top-4 right-4 bottom-4 left-4 translate-x-0 translate-y-0 w-auto h-auto max-w-none sm:max-w-none border-0 bg-transparent p-0 shadow-none overflow-hidden">
            <DialogHeader className="sr-only">
              <DialogTitle>Full Screen Flowchart</DialogTitle>
              <DialogDescription>
                Expanded flowchart viewer for the currently selected decision
                path.
              </DialogDescription>
            </DialogHeader>
            <div className="h-full w-full overflow-hidden rounded-xl border bg-background p-4 shadow-2xl">
              <Flowchart
                paths={paths}
